...
export function ImportWalletDialog({
  ...
  const sellCount = allTrades.length - buyTrades.length;
  ...
  const handleConfirm = (): void => {
    const selected = buyTrades.filter((t) => selectedIds.has(t.eventId));
    if (selected.length === 0) {
      toast.error("Select at least one trade to import");
      return;
    }
    // Send only the fields the server accepts — giftSlug is re-derived server-side
    confirmMutation.mutate({
      trades: selected.map((t) => ({
        giftName: t.giftName,
        ...
        {step === "scan" && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Scan your TON wallet history for Telegram gift purchases. Only buy-side
                marketplace transactions are imported (price available).
              </DialogDescription>
            </DialogHeader>
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {buyTrades.length !== 1 ? "s" : ""} in {eventsFetched} wallet events.
                {sellCount > 0 &&
                  ` (${sellCount} sell transaction${sellCount !== 1 ? "s" : ""} detected but not imported — sell price tracking not yet supported)`}
